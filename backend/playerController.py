...
def get_player_fpl_fixtures(id):
    if id is not None:
        url = 'https://fantasy.premierleague.com/api/element-summary/{}/'.format(id)
        r = requests.get(url)
        json = r.json()
...
async def get_team_xGA(match, team):
...
def get_fpl_opponent_strength(id):
    url = 'https://fantasy.premierleague.com/api/bootstrap-static/'
    r = requests.get(url)
    json = r.json()
    teams = json['teams']
...
def main():
    ...
    task1 = loop.create_task(get_player(player_name))
    player_id, player_team = loop.run_until_complete(task1)

    task2 = loop.create_task(get_player_matches(player_id))
    matches = loop.run_until_complete(task2)

    #Get fpl-related data
    fpl_player_id = get_fpl_player_id(player_name)
    fpl_matches = get_player_fpl_matches(fpl_player_id)
    fpl_fixtures = get_player_fpl_fixtures(fpl_player_id)
    ...
    #Team xGA
    task3 = [get_team_xGA(match, player_team) for match in fpl_matches[len(fpl_matches)-games_considered:len(fpl_matches)]]
    ...
    #Get next opponent team(s) strength
    next_opponents = 1
    ...
    next_opponent_attack_strength = np.median([float(get_fpl_opponent_strength(match['team_a'])[1]) if match['is_home'] is True else float(get_fpl_opponent_strength(match['team_h'])[1]) for match in fpl_fixtures[:next_opponents]])
    next_opponent_defense_strength = np.median([float(get_fpl_opponent_strength(match['team_a'])[3]) if match['is_home'] is True else float(get_fpl_opponent_strength(match['team_h'])[2]) for match in fpl_fixtures[:next_opponents]])
